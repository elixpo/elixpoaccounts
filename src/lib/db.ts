...
export async function createUser(
  db: D1Database,
  { id, email, passwordHash, displayName }: { id: string; email: string; passwordHash?: string; displayName?: string }
) {
  ...
  return await stmt.bind(id, email, passwordHash || null, displayName || null).run();
}
